...
const internal_ip = computed((): string => {
	return flightboxStatusStore.internal_ip;
});
const uptimeApp = computed((): number => {
	return flightboxStatusStore.uptime_app;
});
const number_tracked_flight = computed((): number => {
	return aircraftStore.number_current_flights;
});
		
const screen_on = computed((): boolean => {
	return flightboxStatusStore.screen_on;
});
const uptime = computed((): number => {
	return flightboxStatusStore.uptime;
});
const uptimeWs = computed((): number => {
